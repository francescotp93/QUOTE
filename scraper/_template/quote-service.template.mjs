// ═══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE scraper "PORTALE PROPRIO" (compagnia esterna a Plurima).
//  SCAFFOLD: non importato da nulla. Copialo in scraper/<nome>/quote-service.mjs.
...
//
//  Personalizza tutti i punti marcati  // TODO[ADAPTER]
//  Mantieni i NOMI degli endpoint standard (/status /login /hub /hubveicolo /api
//  /preventivo /explore /sniff*) così i proxy generici di server/fonti.js funzionano
//  senza modifiche.  Output: i formati NORM di docs/ARCHITETTURA-MULTICOMPAGNIA.md §2.3.
// ═══════════════════════════════════════════════════════════════════════════════
...
// TODO[ADAPTER] identità della fonte e del servizio
const FONTE_ID    = process.env.FONTE_ID || 'c-compagnia-x';   // id nel Pannello Fonti (__custom)
const FONTE_MATCH = /compagnia.?x/i;                            // fallback di match sul nome
const DEFAULT_LOGIN = 'https://portale.compagniax.it/login';
const PORT    = Number(process.env.PORT || 4400);              // 4400/:96/5903, poi 4500/:95/5904 …
const NOME    = 'Compagnia X';
const log = (...a) => console.log(new Date().toLocaleTimeString('it-IT'), '[compagniax]', ...a);
...
async function autoLogin() {
  ...
  // TODO[ADAPTER] selettori reali utente/password + submit del portale
  ...
  // TODO[ADAPTER] gestire 2FA: TOTP (vedi allianz totpCode) oppure passcode/OTP (vedi italiana enterPasscode)
  return !isLoginUrl(page.url()) && !(await hasPasswordField());
}
async function ensureLogin() {
  if (await loggedIn()) return true;
  if (await autoLogin().catch(() => false)) return true;
  log('Auto-login non riuscito: accedi UNA volta via VNC.');
  return false;
}

// ── ADAPTER: produce i formati NORM (vedi doc §2.3). TODO[ADAPTER] tutti i corpi. ──
async function recuperaVeicolo(targa, situazione = 'Rinnovo', opts = {}) {
  await ensureLogin();
  // TODO[ADAPTER] interroga la banca dati targa del portale (API firmata o form pilotato).
  // Usa /explore e /sniff per scoprire come. Ritorna VeicoloNorm + SituazioneNorm.
...
async function calcolaPremio(dati = {}, garanzie = []) {
  await ensureLogin();
  // TODO[ADAPTER] costruisci il payload nativo del portale dai dati recuperati -> PremioNorm
...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    // /explore /sniff*  -> copia gli strumenti generici da scraper/italiana (riusabili tali e quali)
...
log('=== SERVIZIO ' + NOME + ' ATTIVO (template) ===');
await new Promise(() => {});
