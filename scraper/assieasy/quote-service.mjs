// ─────────────────────────────────────────────────────────────────────────────
//  Assieasy (CRM gestionale) — scraper portale per CATTURA API.
//  Porta 4800, display :92, VNC 5907. Credenziali dal Pannello Fonti (fonte ~ "assieasy").
//  Login semplice utente+password (nessun 2FA). NON è un preventivatore: serve a loggarsi
//  e a REGISTRARE le chiamate API del CRM (Esplora / Cattura manuale via VNC / Analizza),
//  per capirne la logica (clienti, gestione) e valutare cosa riusare sul nostro gestionale.
//  Sessione persistente (userdata su disco) come gli altri scraper.
// ─────────────────────────────────────────────────────────────────────────────
...
const FONTE_ID = process.env.FONTE_ID || 'c-assieasy';
const DEFAULT_LOGIN = 'https://withus.assieasy.com/assieasy/';
const PORT = parseInt(process.env.PORT || '4800', 10);
const log = (...a) => console.log(new Date().toLocaleTimeString('it-IT'), '[assieasy]', ...a);
...
function rawFonte() {
  try {
    ...
    for (const k of Object.keys(cs)) if (/assieasy|assi.?easy|crm/i.test(cs[k].nome || '')) return cs[k];
    return {};
  } catch { return {}; }
}
function safeLoginUrl(raw) {
  ...
  if (/[?&](code|state|session_state|token|ticket)=/i.test(u)) return DEFAULT_LOGIN; // niente callback monouso
  return u;
}
function creds() {
  const s = rawFonte();
...
const origin = (u) => { try { return new URL(u).origin; } catch { return 'https://withus.assieasy.com'; } };

// ── Browser persistente (flag leggeri + blocco font/media/tracker, come gli altri scraper) ──
...
// ── SNIFF: registra le chiamate API (XHR/fetch/document), saltando rumore/asset ──
...
// Stato login + cache (loggato = sul dominio Assieasy e SENZA campo password visibile).
let BUSY = false;
...
async function loggedIn() {
  ...
  await ensurePage();
  let u = page.url() || '';
  if (!/assieasy/i.test(u)) { await page.goto(creds().loginUrl, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(() => {}); await page.waitForTimeout(2500); u = page.url() || ''; }
  const hasPw = await hasPasswordField();
  const r = /assieasy/i.test(u) && !hasPw && !isLoginUrl(u);
  setLogged(r);
  return r;
}

// ── LOGIN: utente + password, submit (bottone Accedi/Login/Entra o Invio). Sessione persistente. ──
async function doAccedi() {
  if (BUSY) return STATE;
  BUSY = true;
  try {
    setState('credenziali', 'Apro Assieasy e invio le credenziali…', true);
    await ensurePage();
    const c = creds();
    ...
    // utente: primo input email/text/user visibile
    ...
    // esito (può esserci un redirect post-login)
    ...
    return setState('non_loggato', 'Login non riuscito: controlla utente/password (o c\'è un passaggio extra da mappare via VNC).');
  } catch (e) { return setState('error', e.message); }
  finally { BUSY = false; }
}

// Avvio: se la sessione persiste sono già dentro; altrimenti aspetto l'Accedi da Fonti.
(async () => {
  try { await ensurePage(); if (await loggedIn()) { setState('loggato', 'Sessione attiva'); log('sessione persistente attiva ✅'); } else { setState('pronto', 'Pronto: avvia il login da Fonti'); log('PRONTO al login'); } }
...
http.createServer(async (req, res) => {
  ...
  try {
    if (u.pathname.startsWith('/status')) {
      ...
      const c = creds();
      return res.end(JSON.stringify({ url: page.url(), loggato, login_step: STATE.step, login_running: STATE.running, ha_credenziali: !!(c.username && c.password), login_msg: STATE.msg || '' }));
    }
    ...
    // Cattura manuale via VNC: avvia/ferma la registrazione delle API mentre usi Assieasy a mano.
    if (u.pathname.startsWith('/sniff')) {
      const act = u.searchParams.get('act') || '';
      if (act === 'start') { sniffStart(); return res.end(JSON.stringify({ ok: true, msg: 'registrazione avviata' })); }
      if (act === 'stop') { const buf = sniffStop().map(e => ({ k: e.kind, m: e.method, s: e.status, url: e.url, body: String(e.body || '').slice(0, 4000) })); return res.end(JSON.stringify({ ok: true, n: buf.length, captured: buf }, null, 2)); }
      return res.end(JSON.stringify({ ok: false, error: 'act=start|stop' }));
    }
