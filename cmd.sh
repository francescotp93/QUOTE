...
for i in $(seq 1 26); do h=$(git rev-parse --short HEAD); [ "$h" = "9c9f358" ] && { echo "deploy giro $i"; break; }; sleep 5; done
...
PY
echo "=== chiamata PREMIO / risposte col prezzo ==="
...
print("\n=== risposte col prezzo ===")
for c in (d.get("chiamate") or []):
  if "←" in c and c.get("body") and re.search(r'"(premium|amount|price|netAmount|grossAmount|totalPremium|importo|rcaPremium)"|\b\d{2,4}[.,]\d{2}\b', c.get("body","")):
    u=c.get("url","")
    if any(k in u for k in ['ghost','istat','searchProduct','set/','new/','infobike','getdetail','getversions']): continue
    print(f"\n[{c.get('t')}] {c.get('←')} {u}"); print("  resp:",c["body"][:1600])
PY
