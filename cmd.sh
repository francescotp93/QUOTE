...
try:
  ...
  print('hasToken:', d.get('hasToken'), 'casaInit:', (d.get('casaInit') or {}).get('status'))
  cc=d.get('casaControlli') or {}; print('controlliDeroga.status:', cc.get('status'), 'err:', (cc.get('err') or '')[:200])
  cq=d.get('casaQuot') or {}
  print('casaQuot.status:', cq.get('status'), 'len:', cq.get('len'), 'err:', (cq.get('err') or '')[:200])
