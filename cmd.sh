...
timeout 170 curl -s --max-time 165 "http://127.0.0.1:4400/premio?targa=GY263BY&nascita=17/07/1993&debug=1" 2>&1 | python3 -c "import json,sys; d=json.load(sys.stdin); print('expand:',[l for l in (d.get('log') or []) if l.startswith('expand')])" 2>/dev/null
