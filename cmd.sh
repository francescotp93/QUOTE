...
for i in $(seq 1 24); do git -C /opt/withus-backend log --oneline -1 2>/dev/null | grep -q "diagnostica datepicker" && break; sleep 5; done
...
for l in (d.get('log') or []):
    s=str(l)
    if 'dpk' in s.lower() or 'data ultima voltura' in s.lower(): print('  ',s[:200])
" 2>&1 | head -10
