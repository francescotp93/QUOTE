# IDEMPOTENTE: nessuna rigenerazione. Allinea posta_config alla chiave che il
# processo node usa realmente (autorevole per la validazione dell'endpoint).
...
echo "DIGEST http: $(curl -s -o /dev/null -w '%{http_code}' "https://api.withusassicurazioni.it/mail/digest?key=$K&filtro=oggi" --max-time 60)"
