...
echo "== HDI quote debug (genera screenshot) GY263BY =="; T0=$(date +%s)
timeout 170 curl -s --max-time 165 "http://127.0.0.1:4400/premio?targa=GY263BY&nascita=17/07/1993&debug=1" > /dev/null 2>&1
echo "  quote done ($(($(date +%s)-T0))s)"
ls -la /tmp/hdi-gar.png 2>&1
echo "== BASE64_PNG_START =="
base64 -w0 /tmp/hdi-gar.png 2>/dev/null | head -c 1400000
echo ""
echo "== BASE64_PNG_END =="
echo "---fine---"
